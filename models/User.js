    ...
    tokens: [
      {
        name: {
          type: String,
          required: true,
          enum: ["auth", "email_verification", "password_reset", "api", "other"]
        },
        token: {
          type: String,
          required: true,
...
userSchema.methods.saveToken = async function (name, token, expiresIn = "7d", metadata = {}) {
...
userSchema.methods.getToken = function (name) {
  const tokenObj = this.tokens.find(t => t.name === name && t.expiresAt > new Date());
...
userSchema.methods.generateAndSaveAuthToken = async function () {
  ...
  return await this.saveToken("auth", token, process.env.JWT_EXPIRES_IN || "7d", {
    device: "web",
    createdAt: new Date()
  });
};
